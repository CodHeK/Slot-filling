...
class CustomEmbedding:
	def __init__(self, config={}):
		...
		if self.pre_trained == None:
			...
			log("Loading pre-trained " + str(self.pre_trained) + " model...")

		# Check if word embeddings already built
		if not os.path.isfile('embeddings/' + Config.DATA_FILE + '_embeddings.txt'):
			self.model = self.train()
			self.save()
			log("Saved trained word embeddings at (embeddings/" + str(Config.DATA_FILE) + "_embeddings.txt)")
		else:
	...
	def save(self):
		filename = Config.DATA_FILE + '_embeddings.txt'
		self.model.wv.save_word2vec_format('embeddings/' + filename, binary=False)

	def EmbeddingLayer(self):
		embedding_index = {}
		file = open(os.path.join('', 'embeddings/' + Config.DATA_FILE + '_embeddings.txt'), encoding='utf-8')
		for line in file:
			values = line.split()
			word = values[0]
